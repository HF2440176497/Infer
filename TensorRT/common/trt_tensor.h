...
namespace TRT {
    ...
    public:
        ...
        explicit Tensor(nvinfer1::DataType         dtype = nvinfer1::DataType::kFLOAT,
                        std::shared_ptr<MixMemory> data = nullptr, int device_id = CURRENT_DEVICE_ID);
        explicit Tensor(int n, int c, int h, int w, nvinfer1::DataType dtype = nvinfer1::DataType::kFLOAT,
                        std::shared_ptr<MixMemory> data = nullptr, int device_id = CURRENT_DEVICE_ID);
        explicit Tensor(int ndims, const int* dims, nvinfer1::DataType dtype = nvinfer1::DataType::kFLOAT,
                        std::shared_ptr<MixMemory> data = nullptr, int device_id = CURRENT_DEVICE_ID);
        ...
        Tensor&      set_stream(cudaStream_t stream, bool owner = false) {
            stream_ = stream;
            stream_owner_ = owner;
            return *this;
        }
